...
export const Capabilities: React.FC<{ capabilities: Capability[] }> = ({ capabilities }) => (
            ...
            <div className="grid md:grid-cols-3 gap-6">
                {capabilities.map((cap, i) => (
                    <div key={i} className="glass-card rounded-2xl p-8">
                        <h3 className="text-xl font-bold mb-3 text-accent">{cap.title}</h3>
...
export const Architecture: React.FC<{ architecture: ArchType }> = ({ architecture }) => (
                    ...
                    <div key={i} className="relative">
                        ...
                        <div className="glass-card rounded-xl p-6 flex flex-col md:flex-row items-center gap-8">
                            <div className="w-full md:w-64 flex-shrink-0">
                                <span className="text-[10px] uppercase tracking-[0.2em] text-white/30 block mb-1">Layer {architecture.layers.length - i}</span>
    ...
    <section id="governance" className="py-24 bg-black border-t border-white/10">
        <div className="max-w-7xl mx-auto px-6">
            <div className="grid lg:grid-cols-2 gap-16 items-center">
                <div>
                    <h2 className="text-4xl font-bold mb-6">Ethical & Operational Governance</h2>
                    ...
                    <div className="space-y-8">
                        {governance.map((gov, i) => (
                            <div key={i} className="flex gap-6">
                                <div className="w-12 h-12 rounded-full bg-accent/10 border border-accent/30 flex items-center justify-center flex-shrink-0 text-accent">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-6 h-6">
                                        <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75 11.25 15 15 9.75m-3-7.036A11.959 11.959 0 0 1 3.598 6 11.99 11.99 0 0 0 3 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285Z" />
                                    </svg>
                                </div>
                                <div>
                                    <h3 className="font-bold mb-2">{gov.title}</h3>
                                    <p className="text-sm text-white/40">{gov.description}</p>
                                </div>
                            </div>
                        ))}
                    </div>
